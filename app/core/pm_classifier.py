...
[데이터 맵핑 지침]
- source_page: 텍스트에 포함된 "--- 페이지 X ---" 구분선을 참고하여 해당 항목이 발견된 정확한 페이지 번호(정수)를 기입하세요.
- note(비고): 항목을 추출한 근거가 되는 **원본 텍스트 문장(원문 그대로)이나 표의 셀 내용**을 구체적으로 인용하여 기록하세요. (이후 사용자가 근거를 쉽게 역추적하기 위함)
